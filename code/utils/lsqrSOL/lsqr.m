...
% 02 Dec 2011: Sergey Bochkanov reports trouble on some toy examples.
%              A=1, b=1, damp=0, [A; damp]x ~= [b; 0] gave test2=NaN
%                 because rnorm=0 during iteration 1.
%                 Must safeguard test2 = arnorm/(anorm*rnorm).
%              A=1, b=1, damp=1, [A; damp]x ~= [b; 0] gave test2=NaN
%                 because anorm=0 inadvertently when beta2=0.
%                 Must update anorm before testing if beta > 0.
%-----------------------------------------------------------------------
...
%                alfa*v  =  A'*u  -  beta*v.
      ...
      % 02 Dec 2011: Do previous line before the following "if".
      if beta > 0
         u     = (1/beta) * u;
...
%     02 Dec 2011: Safeguard test2 = arnorm/(anorm*rnorm) when rnorm=0.

      test1   =   rnorm / bnorm;
      test2   =   arnorm/( anorm * (rnorm + 1e-50));
      test3   =       1 / acond;
